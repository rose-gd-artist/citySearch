...
const displayCities = (bigTown) => {
    const cityListings = bigTown.map(bigTown => {
        return `<div class="cityName">City: ${bigTown.name}</div>
                <div class="cityPop">Population: ${commafy(bigTown.population)}</div>
...
function cityFiltered(event) {
    ...
    if(answer === sVal){
        let searchEntered = selectedCity.map((selectedCity) => {
            ...
            return `<div class="cityName">City: ${selectedCity.name}</div>
                <div class="cityPop">Population: ${commafy(selectedCity.population)}</div>
                <div class="cityLandmark">Landmark: ${selectedCity.landmark}</div>`;
        });
        result.innerHTML += searchEntered;
    }
};
